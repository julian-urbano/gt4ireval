---
title: "gt4ireval: An R package to Measure the Reliability of an Information Retrieval Test Collection with Generalizability Theory"
...
Finally, function `gt2data` can be used to map stability indicators from Generalizability Theory onto well-known indicators such as Kendall $\tau$ correlation or sensitivity (see [@Urbano2013] for details).
...
```{r}
adhoc3.d <- dstudy(adhoc3.g, queries = seq(10, 100, 10))
gt2data(Erho2 = adhoc3.d$Erho2, Phi = adhoc3.d$Phi)
```

Similarly, we can use the returned `gt2data` object in subsequent computations. For instance, we can plot the estimated $\hat\tau$ correlation as a function of the query set size.

```{r}
xx <- seq(10, 200, 5)
adhoc3.d <- dstudy(adhoc3.g, queries = xx)
adhoc3.i <- gt2data(Erho2 = adhoc3.d$Erho2)
names(adhoc3.i)
plot(xx, adhoc3.i$tau,
     yaxs = "i", ylim = c(0.5, 1), lwd = 2, type = "l",
     xlab = "Number of queries", ylab = "Kendall rank correlation")
lines(xx, gt2data(Erho2 = adhoc3.d$Erho2.lwr)$tau) # lower confidence limit
lines(xx, gt2data(Erho2 = adhoc3.d$Erho2.upr)$tau) # upper confidence limit
grid()
```
...
[^1]: For general information on how to read data in `R`, the reader is referred to the [R Data Import/Export manual](http://cran.r-project.org/manuals.html).
